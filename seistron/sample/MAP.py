...
def estimate_maximum_a_posteriori(obs: ArrayLike, x_init: ArrayLike,
                                  model: Callable, model_kwargs: dict,
                                  optimizer_kwargs: dict, method='BFGS'
                                  ) -> OptimizeResults:
    """
    A wrapper for jax.scipy.optimize.minimize.
    Gives a MAP estimate for a forward model given observations
    obs: The observations.
    model: The forward function for computing predictions.
    model_kwargs: key word arguments for calling the model
    optimizer_kwargs: key word arguments for
    calling jax.scipt.optimize.minimize
    """
